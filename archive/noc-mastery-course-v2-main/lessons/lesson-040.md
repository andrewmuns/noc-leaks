# Lesson 40: SIP Methods — INVITE, REGISTER, BYE, and Beyond
**Module 1 | Section 1.10 — SIP Protocol Deep Dive**
**⏱ ~8 min read | Prerequisites: Lesson 37**
...
🔧 **NOC Tip:** If you see repeated 200 OK retransmissions in SIP traces, the ACK isn't getting through. Check for firewall issues, NAT problems, or routing errors that prevent the ACK from reaching the UAS.
...
🔧 **NOC Tip:** If you see a trunk marked as "down" despite the customer claiming their system is running, check if OPTIONS packets are getting through. Firewalls that allow INVITE but block OPTIONS (because they don't recognize the method) will cause the health check to fail while calls still work.
