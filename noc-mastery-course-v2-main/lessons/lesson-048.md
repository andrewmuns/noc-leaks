# Lesson 48: Call Transfer — REFER and Replaces
...
🔧 **NOC Tip:** Attended transfer failures often come from URL encoding issues in the `Replaces` header. The Call-ID, semicolons, and tags must be properly escaped. If transfers work for blind but fail for attended, check the REFER message for properly encoded Replaces parameters.
...
🔧 **NOC Tip:** When a customer reports "transfers don't work," first determine the transfer type (blind vs. attended) and the phone model/PBX vendor. Then check:
