# Lesson 37: Packet Loss — Causes, Effects, and Measurement
**Module 1 | Section 1.9 — Jitter, Latency, and Call Quality**
**⏱ ~8 min read | Prerequisites: Lesson 33, 34**
...
🔧 **NOC Tip:** When reviewing loss metrics, always look at the **loss pattern**, not just the percentage. RTCP reports aggregate loss over the reporting interval — they can't distinguish random from burst. You need packet captures to see the actual pattern.
...
🔧 **NOC Tip:** When you suspect path-level loss, don't rely solely on ICMP ping. Some networks prioritize or deprioritize ICMP differently than UDP. Use `iperf3 -u` with a bandwidth matching voice traffic (~100 kbps per stream) for a realistic test.
