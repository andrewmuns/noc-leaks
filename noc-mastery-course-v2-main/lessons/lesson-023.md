# Lesson 23: DNS Troubleshooting — dig, nslookup, and Common Failures
**Module 1 | Section 1.5 — DNS**
**⏱ ~6min read | Prerequisites: Lessons 19, 20**
---
...
🔧 **NOC Tip:** If `+trace` stalls at a particular delegation level, you've found your problem. A broken delegation between the registrar and the authoritative nameserver is a common cause of "DNS works from some places but not others."
...
🔧 **NOC Tip:** When walking a customer through troubleshooting on Windows, use `nslookup` — it's always available. For your own analysis on Linux, always prefer `dig`.
...
🔧 **NOC Tip:** If `dig +cd` returns an answer but a normal `dig` returns SERVFAIL, you have a DNSSEC problem. The zone's DNSSEC signatures are broken or expired.
...
🔧 **NOC Tip:** DNSSEC failures are almost never something Telnyx causes — they're at the customer's DNS provider. But they affect our service because *our* resolvers validate DNSSEC. Point the customer to https://dnsviz.net/ to visualize their DNSSEC chain.
...
🔧 **NOC Tip:** When a customer says "calls work to some destinations but not others," check if the failing destination has broken SRV records. Many small VoIP providers have misconfigured DNS. Use `dig _sip._udp.their-domain.com SRV` to verify.
