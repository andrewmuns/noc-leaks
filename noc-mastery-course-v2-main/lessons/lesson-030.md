# Lesson 30: SIP ALG, Session Border Controllers, and Media Anchoring

**Module 1 | Section 1.7 — NAT**
**⏱ ~6 min read | Prerequisites: Lesson 27**
...
🔧 **NOC Tip:** At Telnyx, the B2BUA acts as the SBC. When you see calls from a customer behind NAT, verify the call flow shows the B2BUA handling both legs. Call-site-finder can tell you which B2BUA handled a specific call.
...
🔧 **NOC Tip:** If a customer reports audio that works for 30 seconds then "switches to one-way," check for latch timeout. Disable silence suppression (comfort noise instead) to keep RTP flowing and the latch alive.
...
🔧 **NOC Tip:** When escalating one-way audio, always provide: direction of break, call IDs, source/destination IPs, and confirm if it's consistent or intermittent. This saves hours of debug time.
