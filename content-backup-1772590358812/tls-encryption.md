...
> **💡 NOC Tip:**  a SIP device only supports TLS 1.0 or 1.1 (deprecated), it will fail to connect to servers that require TLS 1.2+. The handshake will fail immediately. In Wireshark, you'll see a ClientHello with an old TLS version followed by a fatal alert or connection reset. This is common with older IP phones and ATAs.
...
> **💡 NOC Tip:** bug certificate issues with `openssl s_client`:
```bash
...
> **💡 NOC Tip:** en SIP over TLS connections fail completely (no SIP traces at all), the problem is always in the TLS handshake — before SIP starts. Capture at the TCP level. Common causes: expired certificate, untrusted CA, TLS version mismatch, cipher suite mismatch, or SNI mismatch. `openssl s_client` is your best friend here.
