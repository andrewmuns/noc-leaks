...
> **💡 NOC Tip:**  WebRTC calls connect but no audio flows, check DTLS handshake success in Chrome's `chrome://webrtc-internals`. Failed DTLS = no SRTP keys = no media.
...
> **💡 NOC Tip:** bRTC to SIP calls traverse Telnyx's voice-sdk-proxy. This handles Opus→G.711 transcoding and all the complex signaling translation between WebSocket/JSON and SIP/SDP.
